...
class ParlayBuilder:
    ...
    def format_message(self, parlay: List[Dict], total_analyzed: int) -> str:
        ...
        for i, leg in enumerate(parlay, 1):
            ...
            lines.append(f"  Kickoff: {format_kickoff(leg.get('commence_time'))}")
        ...
        over_count = sum(1 for leg in parlay if leg["bet_type"] == "Over 2.5")
        dc_count = sum(1 for leg in parlay if leg["bet_type"].startswith("Double Chance"))
        x12_count = len(parlay) - over_count - dc_count
        ...
        lines.append(f"- Bet types: {x12_count} x 1X2, {dc_count} x Double Chance, {over_count} x Over 2.5")
