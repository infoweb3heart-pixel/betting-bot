...
# Instead of a fixed league list, the bot now asks The Odds API for every
# soccer league that's CURRENTLY IN SEASON and polls all of them.
#
# COST WARNING: calling /sports (to get the league list) is free, but each
# league's /odds call still costs `markets_requested x regions_requested`
# credits (2 credits per league with the settings below). At any given time
# there are usually 15-30 soccer leagues in season worldwide, so a single
# /parlay run can burn 30-60+ credits. The free tier only gives 500/month —
# at that rate you may only get ~8-15 runs before hitting the cap. If you
# want more runs per month, go back to a short hand-picked LEAGUES list
# instead (ask me and I'll swap it back).
USE_ALL_ACTIVE_SOCCER_LEAGUES = True

# Fallback list used only if the dynamic /sports fetch fails for some reason.
FALLBACK_LEAGUES = [
...
def fetch_active_soccer_leagues(api_key):
    """
    Ask The Odds API which soccer leagues are currently in season.
    This call is FREE (does not cost quota) — only the per-league /odds
    calls that follow cost credits.
    """
    ...
    leagues = [
        {"key": s["key"], "label": s.get("title", s["key"])}
        for s in all_sports
...
def fetch_all_matches(api_key):
    """Fetch and flatten odds across every in-season soccer league."""
    if USE_ALL_ACTIVE_SOCCER_LEAGUES:
        try:
            leagues = fetch_active_soccer_leagues(api_key)
            print(f"[odds] Found {len(leagues)} active soccer leagues.")
        except requests.RequestException as err:
            print(f"[odds] Failed to fetch league list, using fallback list: {err}")
...
def format_parlay_message(result):
    if not result["ok"]:
        ...
        return (
            ...
            "Try again later, or try a wider HOURS_WINDOW near the top of main.py."
        )
    ...
    try:
        ...
        if not matches:
            bot.send_message(
                ...
                "fixture windows, or if today's slate is thin across every active soccer league — "
                "try again later, or widen HOURS_WINDOW near the top of main.py.",
            )
            return
...
def start_bot_polling():
    # Clear any leftover webhook/connection before polling. If a previous
    # instance (e.g. from a redeploy, or a local test run) is still holding
    # the connection, Telegram will return a 409 Conflict — this, plus the
    # short delay below, gives the old one time to drop off first.
    try:
        bot.remove_webhook()
    except Exception as err:  # noqa: BLE001
        print(f"[startup] remove_webhook() failed (usually harmless): {err}")

    time.sleep(3)

    print("Telegram bot is polling for messages...")
    # none_stop=True + a real interval/timeout means transient 409s from a
    # slow-to-die previous instance get retried automatically instead of
    # crashing the thread.
